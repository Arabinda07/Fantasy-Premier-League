...
- **Any change to `model/` or `knowledge/`**: Follow [`.agents/rules/okf-discipline.md`](.agents/rules/okf-discipline.md) for ground-truth sources and verification gates.
...
- **Python / Model tasks**: Run `pytest` (or targeted `pytest model/test_<feature>.py`), plus the attesters: `python knowledge/references/attesters/verify_schema.py` and `verify_solver.py` (default to the GW2 fixtures).
...
**Definition of Done**: A task is complete only when the implementation is in place, relevant local verification passes, and any regressions caused by your changes have been resolved.
...
- Strict compliance with [`DESIGN.md`](file:///e:/Fantasy-Premier-League/DESIGN.md). New UI follows **Direction J, "Matchday Radio Wire"** ([`ROUND-2-CONTEXT.md`](file:///e:/Fantasy-Premier-League/ROUND-2-CONTEXT.md)): warm charcoal ground, cream/zinc letterforms, hierarchy through size and weight.
- Tokens only: use the CSS custom properties in `:root` of `frontend/src/styles/index.css`. Hex values live only there.
- No decorative borders, boxes, pills or badge stacks. Colour is allowed only for FDR 1–5 and up/down deltas, always paired with a number or arrow.
- `.surface-scope-*` classes are tab namespaces only, with no per-tab accent.
...
- Reciprocal Registration: When creating a new repeatable UI component or variant, document it in the [`DESIGN.md`](file:///e:/Fantasy-Premier-League/DESIGN.md) catalog in the same change. (`ComponentStudio.jsx` is the Forecaster tab, not a component registry.)
- Frontend tests: `npm test` inside `frontend/` runs the `node:assert` suites in `src/utils/`.
